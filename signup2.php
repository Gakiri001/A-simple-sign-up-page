...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["password"])) {
        ...
        if (!preg_match("/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@#$%^&+=!])(?=.*[a-zA-Z\d@#$%^&+=!]).{8,}$/", $password)) {
            $passwordErr = "Enter a strong password";
        }
    }
...
// Values to be taken when the submit button is clicked
if (isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO admin (schoolname, schoolcode, county, principal, email, password) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $schoolname, $schoolcode, $county, $principal, $email, $password);
    $stmt->execute();
...
<body>
    <h2>
        Sign up to create an account for Admin
    </h2>
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
        <fieldset>
            ...
            <input class="i" type="text" name="schoolname" value="<?php echo $schoolname; ?>">
            <span class="Error">* <?php echo $schoolnameErr; ?></span>
            <br><br>
            School Code: <br>
            <input class="i" type="text" name="schoolcode" value="<?php echo $schoolcode; ?>">
            <span class="Error">* <?php echo $schoolcodeErr; ?></span>
            <br><br>
            School's county: <br>
            <input class="i" type="text" name="county" value="<?php echo $county; ?>">
            <span class="Error">* <?php echo $countyErr; ?></span>
            <br><br>
            School's Principal: <br>
            <input class="i" type="text" name="principal" value="<?php echo $principal; ?>">
            <span class="Error">* <?php echo $principalErr; ?></span>
            <br><br>
            School Email Address: <br>
            <input class="i" type="text" name="email" value="<?php echo $email; ?>">
